...
import { GHANA_REGIONS_OFFICES, GHANA_REGIONS, getOfficesForRegion } from "@/hooks/useAdminProfile";
...
const SerialGenerator = ({ onStockChanged }: Props) => {
  ...
  const quantity = useMemo(() => Math.max(0, endRange - startRange + 1), [startRange, endRange]);
  ...
  const generatedSerials = useMemo(() => {
    ...
    return serials;
  }, [prefix, startRange, endRange, padLength, quantity]);

  const canGenerate = quantity > 0 && quantity <= 10000 && prefix.trim() && selectedRegion && (assignToRegion || selectedOfficeId);
  ...
  const handleConfirm = async (password: string, reason: string) => {
    ...
    try {
      ...
      toast.success(`${quantity} serial(s) generated and added to stock!`);
      onStockChanged();
      setShowPreview(false);
    } catch (err: any) {
      throw err; // Let AdminPasswordConfirm handle the error display
    } finally {
      setGenerating(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="bg-card rounded-xl border border-border p-6 space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="flex items-center justify-between gap-4 bg-muted/30 rounded-lg p-3">
            <div>
              ...
              <p className="text-2xl font-bold text-primary">{quantity > 10000 ? "Too large" : quantity.toLocaleString()}</p>
            </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <Button variant="outline" onClick={() => setShowPreview(!showPreview)} disabled={quantity <= 0 || quantity > 10000}>
            <Eye className="h-4 w-4 mr-1" /> {showPreview ? "Hide Preview" : "Preview Serials"}
          </Button>
          <Button onClick={handleGenerate} disabled={!canGenerate || generating}>
            <Wand2 className="h-4 w-4 mr-1" />
            {generating ? "Generating..." : `Generate ${quantity.toLocaleString()} Serial(s)`}
          </Button>
        </div>

        {/* Preview */}
        {showPreview && generatedSerials.length > 0 && (
          <div className="border border-border rounded-lg p-4 space-y-2 max-h-60 overflow-y-auto">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-card-foreground">Preview — {generatedSerials.length} serial(s)</p>
              <Button variant="ghost" size="sm" onClick={() => setShowPreview(false)}>Close</Button>
            </div>
            <div className="flex flex-wrap gap-1.5">
              {generatedSerials.slice(0, 100).map(s => (
                <Badge key={s} variant="outline" className="font-mono text-xs">{s}</Badge>
              ))}
              {generatedSerials.length > 100 && (
                <Badge variant="secondary" className="text-xs">+{generatedSerials.length - 100} more</Badge>
              )}
            </div>
          </div>
        )}
      ...
      <AdminPasswordConfirm
        ...
        description={`You are about to generate ${quantity.toLocaleString()} serial number(s) and add them to stock. This action will be logged.`}

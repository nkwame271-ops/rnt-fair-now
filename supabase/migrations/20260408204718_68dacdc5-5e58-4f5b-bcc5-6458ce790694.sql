...
-- Atomic pair-based serial assignment function
CREATE OR REPLACE FUNCTION public.assign_serials_atomic(
  p_pairs jsonb,        -- array of { serial_number, card_ids: [id1, id2] }
  p_office_id text,
  p_office_name text,
  p_assigned_by uuid
)
...
BEGIN
  ...
  LOOP
    ...
    -- Verify we found stock rows
    IF NOT EXISTS (SELECT 1 FROM rent_card_serial_stock WHERE serial_number = v_serial) THEN
      RAISE EXCEPTION 'Serial % not found in stock', v_serial;
    END IF;
    ...
    -- All validations passed for this pair — perform assignment

    -- Update both stock rows
    UPDATE rent_card_serial_stock
    SET status = 'assigned',
        assigned_to_card_id = v_card_ids[1],  -- reference first card
        assigned_at = now(),
...
-- Atomic unassign function
CREATE OR REPLACE FUNCTION public.unassign_serial_atomic(

...
GRANT EXECUTE ON FUNCTION public.lookup_serial_details(text[]) TO authenticated, service_role;
...
BEGIN
  ...
  FOREACH v_serial IN ARRAY p_serials LOOP
    ...
    ELSIF p_target_kind = 'regional' THEN
      UPDATE public.rent_card_serial_stock
      SET stock_type = 'regional',
          office_name = '',
          region = p_target_region,
          office_allocation_id = NULL
      WHERE serial_number = v_serial;
    ELSE
      UPDATE public.rent_card_serial_stock
...
GRANT EXECUTE ON FUNCTION public.move_serials_atomic(text[], text, text, text, text, uuid, text) TO authenticated, service_role;

...
BEGIN
  ...
  IF p_agent_user_id IS NOT NULL THEN
    ...
    SELECT p_agent_user_id, v_sub.subscriber_user_id, v_sub.subscriber_role,
           'Premium Service — property ' || v_sub.property_id::text, auth.uid(), true
    WHERE NOT EXISTS (
    ...
    VALUES
      (p_agent_user_id, 'Premium property assigned', 'A Premium Service property has been assigned to you.', '/agent/assigned-properties'),
      (v_sub.subscriber_user_id, 'Premium Service agent updated', 'Your assigned Premium Service agent has been updated.', '/landlord/premium');
  ELSE
    INSERT INTO public.notifications(user_id, title, body, link)
    VALUES (v_sub.subscriber_user_id, 'Premium Service awaiting agent', 'Your Premium Service property is awaiting a new agent assignment.', '/landlord/premium');
  END IF;

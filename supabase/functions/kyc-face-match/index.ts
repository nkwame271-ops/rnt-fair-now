...
Deno.serve(async (req) => {
  ...
  try {
    const { ghanaCardFrontUrl, selfieUrl } = await req.json();

    if (!ghanaCardFrontUrl || !selfieUrl) {
      throw new Error("Both ghanaCardFrontUrl and selfieUrl are required");
    }
    ...
    // Use Lovable AI Gateway with vision model to compare faces
...
- reason: brief explanation of your assessment`
          },
          {
            ...
            content: [
              ...
              { type: "image_url", image_url: { url: ghanaCardFrontUrl } },
